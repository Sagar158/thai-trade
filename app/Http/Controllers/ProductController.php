...
class ProductController extends Controller
{
    ...
    public function getProductsData(Request $request)
    {
        ...
        return DataTables::of($products)
               ->addColumn('checkbox', function($product){
                    return '<input type="checkbox" class="row-checkbox select-product" data-remarks="'.$product->remarks.'" data-productid="'.$product->product_id.'" data-product-code="'.$product->cs_did.'" data-productname="'.$product->name.'" data-w="'.$product->w.'" data-l="'.$product->l.'" data-h="'.$product->h.'" data-product-sku="'.$product->sku.'" data-weight="'.$product->weight.'" data-option="'.$product->option.'" data-type="'.$product->type.'" data-warehouse="'.$product->warehouse.'" data-photo1="'. Storage::url($product->photo1).'" data-photo2="'. Storage::url($product->photo2).'" data-photo3="'. Storage::url($product->photo3).'" data-tWeight="'.round(($product->weight * $product->tpcs), 2).'" data-photo4="'. Storage::url($product->photo4).'" data-tcube="'.$this->calculateTCube($product->l , $product->w , $product->h, $product->tpcs).'">';
               })
               ...
               ->addColumn('sku', function($product){
                ...
                {
                    $for_copy = $product->products;
                }
                $product_str .= '<button style="float:right;" onclick="copyProducts(`' . $for_copy . '`)">C</button></div>';
                return $product_str;
            })
               ...
               ->addColumn('t_cube', function($product){
                    return number_format($this->calculateTCube($product->l , $product->w , $product->h, $product->tpcs), 4);
               })
    ...
    public function calculateTCube($length, $width, $height, $tpcs)
    {
        return (($length * $width * $height * $tpcs) / 1000000);
    }
    ...
    public function store(Request $request)
    {
        ...
        $pro = Product::where('sku' , $request->sku)->get();
        if(count($pro) > 0)
        return response()->json(['message' => 'Barcode already present in database', 'status' => false], 422);
    ...
    public function addToRepack(Request $request)
    {
        ...
        $productRepack = RepackProduct::create([
            ...
            't_cube' => round( (float)$request->tcube , 4 ) ,   #$request->tcube,
    ...
    public function CKWC2TData(Request $request)
    {
        ...
        return DataTables::of($products)
                ...
                ->addColumn('total_weight', function($product){
                    return round(($product->t_weight), 2);
                })
                ->addColumn('length', function($product){
                    return $product->l;
                })
                ...
                ->addColumn('t_cube', function($product){
                    return (isset($product->t_cube) && !empty($product->t_cube)) ? number_format($product->t_cube,4) : '';
                })
                ->addColumn('created_at', function($product){
                    return date('F j, Y h:i:s a', strtotime($product->created_at));
                })
    ...
    public function repackProductsData(Request $request)
    {
        ...
        return DataTables::of($products)
               ->addColumn('checkbox', function($product){
                    return '<input type="checkbox" class="row-checkbox" data-product-id="'.$product->id.'" data-product-code="" data-product-name="" data-product-w="" data-product-l="" data-product-h="" data-product-tpcs="" data-product-sku="" data-product-weight="" data-option="" data-type="" data-warehouse="">';
               })
               ->addColumn('csd_id', function($product){
                    $csd_id = isset($product->customer->CS) ? $product->customer->CS : '';
                    return $csd_id;
